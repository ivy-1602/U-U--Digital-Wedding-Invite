# उदय & उन्नती 
### A digital wedding invitation.

My vahini saheb is getting married into our family on **23 June 2026**. *clapsss

So I built her a website. Dada happens to be in it too.

---

It started as a simple idea... why send a printed card when I could build something that actually moves, glows, plays music, and counts down to the exact moment the pheras begin? One weekend turned into several. The splash screen got flower confetti. The hero got spinning gold mandalas. There's a peacock drawn entirely in SVG markup sitting between sections because I thought it would be nice and then couldn't stop until it had eye-spots and a crown.

This is what happens when a CS student with love for UIUX really lover her vahini saheb.

---

## The people this is for

**Unnati** - my vahini saheb. You are kind and warm and genuinely fun to be around, and I am very glad you said yes. Welcome to the Salunke family. I'm in your team.

**Uday** - my dada. He's fine. He's getting married. Congratulations to him as well.

---

## The events

| | |
|---|---|
| Ring Ceremony | Monday, 22 June · 5:00 PM |
| Sangeet Night | Monday, 22 June · 7:00 PM |
| Haldi | Monday, 22 June · 8:00 PM |
| **Wedding** | **Tuesday, 23 June · 12:30 PM** |

---

## What I built

A fully static site — HTML, CSS, and vanilla JavaScript, no frameworks, no build tools. Every animation is hand-written. Every ornament is inline SVG. The background colour shifts as you scroll through each section, interpolating between crimson and saffron and royal blue and sage green and blush pink. The body background is essentially a mood ring for the wedding schedule.

The music is *Sukh Kalale* on piano. It starts when you tap the splash screen.

Live → **[uday-unnati-wedding-invitation-2026.vercel.app](https://uday-unnati-wedding-invitation-2026.vercel.app/)**

---

## 🎊 A note

I had no option but to make this public on github because i deyployed it on vercel so...Please don't manupulate this for your own. That said, if you're here to learn how the confetti works or why the background colour changes on scroll, poke around all you want. Just don't ship it as yours.

© 2026 Uma Salunke — made with entirely too many radial gradients and zero regrets.

---

## 📫 Contact

**Uma Salunke** · AI & ML Engineering Student

[LinkedIn](https://www.linkedin.com/in/umasalunke7) · [GitHub](https://github.com/ivy-1602) · [Email](mailto:umasalunke7@gmail.com)

---

## Closing Note
I wish for a veryyyy sucessfull marraige for both of youu, i love you guys, i'll pray that no evil eyes touch this divine bond of yours.

---
ps. vahini saheb, I hope every day with dada and us feels like a Saturday 🌸
